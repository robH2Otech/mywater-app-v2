...
export function UVCMetricsCards({ businessMetrics, costSavings, period, realDataAvailable = true }: UVCMetricsCardsProps) {
  const cards = [
    {
      title: "Water Processed",
      value: `${businessMetrics.waterProcessed.toFixed(1)} m³`,
      subtitle: realDataAvailable 
    ...
    },
    {
      ...
      subtitle: realDataAvailable 
        ? `€${costSavings.energyCostSavings.toFixed(2)} cost savings`
        : "Based on real usage",
      icon: Zap,
    ...
    },
    {
      ...
      subtitle: realDataAvailable 
        ? `€${costSavings.waterCostSavings.toFixed(2)} equivalent`
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {cards.map((card, index) => (
        ...
        >
          <Card className={`bg-gradient-to-br ${card.bgColor} border ${card.borderColor} hover:shadow-lg transition-all duration-300 hover:scale-105 ${!realDataAvailable ? 'opacity-75' : ''}`}>
